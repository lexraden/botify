...
class Settings(BaseSettings):
    ...
    # Платные тарифы продавца (app/plans.py, app/payments/subscription.py).
    ...
    #
    # ВНИМАНИЕ при обновлении прода: 2026-09-09 названия тарифов поменялись
    # местами (младший теперь Plus, старший Pro). Если PRO_PRICE_USDT и
    # PLUS_PRICE_USDT заданы в окружении, их значения тоже надо поменять
    # местами — иначе дешёвый тариф начнёт стоить как дорогой.
    plus_price_usdt: float = 20.0
    plus_price_stars: int = 1500
    # Pro = Plus плюс приём оплаты по реквизитам продавца в чате заказа.
    # Звёзды пересчитаны по той же пропорции (1500 звёзд за 20 USDT), чтобы
    # выручка платформы совпадала.
    pro_price_usdt: float = 50.0
    pro_price_stars: int = 3750
    # Срок и напоминание общие для обоих платных тарифов
    pro_period_days: int = 30
    # За сколько дней до конца подписки напомнить продавцу
    pro_reminder_days: int = 3

    # Оплата переводом на реквизиты продавца (тариф Pro, app/services/p2p.py).
    # Своё окно на оплату: счёт Crypto Pay живёт час, а перевод человек делает
    # руками, иногда из другого приложения и не сразу.
    p2p_order_ttl_minutes: int = 180
    # Через сколько часов после «я оплатил» напомнить продавцу подтвердить
    p2p_confirm_reminder_hours: float = 2.0
    ...
    # Legacy с 2026-09-10: внешнюю ссылку «Поддержка» заменил встроенный канал
    # «Связаться с нами» (app/services/feedback.py, пуш от hub-бота админам).
    # Переменная пока не удаляется, чтобы неизвестное окружение не упало;
    # код её больше не читает. История: раньше кнопка вела в hub-бот, то есть
    # покупателя с проблемой по заказу регистрировали продавцом и показывали
    # ему рекламу конструктора ботов.
    support_url: str = ""

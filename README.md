...
- [x] Этап 4 — Mini App: витрина с корзиной (Durger King-флоу), «Мои покупки»,
      кабинет продавца (товары/заказы/статистика); API с initData-авторизацией
...
- [x] Этап 6 — постпродажа: продавец прикрепляет трек/ссылку → бот пересылает покупателю;
      выплаты через Crypto Pay transfer (spend_id-идемпотентность, ежечасный ретрай)
